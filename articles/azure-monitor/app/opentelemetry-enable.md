...
Download the [applicationinsights-agent-3.6.0.jar](https://github.com/microsoft/ApplicationInsights-Java/releases/download/3.6.0/applicationinsights-agent-3.6.0.jar) file.
...
Point the Java virtual machine (JVM) to the jar file by adding `-javaagent:"path/to/applicationinsights-agent-3.6.0.jar"` to your application's JVM args.
...
* **Set via configuration file** - *Java only*
    ...
    Create a configuration file named `applicationinsights.json`, and place it in the same directory as `applicationinsights-agent-3.6.0.jar` with the following content:
