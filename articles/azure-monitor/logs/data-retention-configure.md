...
|[Custom log table](./create-custom-table.md#create-a-custom-table) (`table_CL`)|Custom log table holds logs from [logs ingestion API](./logs-ingestion-api-overview.md), or HTTP data collector API (deprecated).<br> When you delete a custom log table in **Analytics** or **Basic** plans, the table retention remains for fourteen days and changed to workspace retention then. Data remains in the workspace until the end of the retention period and charged accordingly.<br>When you delete a custom table in **Auxiliary** plan, the table retention remains for fourteen days and data is soft deleted. After the fourteen days period, retention is changed to workspace retention, data is permanently deleted, but charge adheres to table retention. |To minimize charges, set [table-level retention](#configure-table-level-retention) to four days before you delete the table.|**Analytics** or **Basic** plans: Create the table with the same name and schema. Data recovery is subjected to table retention.<br>**Auxiliary** plan: Create the table with the same name and schema during the soft delete period.|
