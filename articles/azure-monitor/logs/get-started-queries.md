...
* Following the pipe is the `take` operator. You can run the query without adding `| take 10` but it could return more results than you can reasonably view, up to the [maximum results](../fundamentals/service-limits.md#azure-portal).
...
For example, the data returned by both of the following queries is sorted by the [TimeGenerated column](./log-standard-columns.md#timegenerated-column), in descending order:
