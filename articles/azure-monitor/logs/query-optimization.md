...
- In many cases, operators such as [join](/azure/kusto/query/joinoperator?pivots=azuremonitor) and [summarize](/azure/kusto/query/summarize-operator) lower overall parallelism. Examine using [shuffle](/azure/kusto/query/shuffle-query) when performance is problematic.
