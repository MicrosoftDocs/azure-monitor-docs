...
| Error code 429 | API limits have been exceeded. The limits are currently set, per data collection rule (DCR), to 2 GB of data per minute for both compressed and uncompressed data and 12,000 requests per minute. For the current values, see [Logs ingestion API limits](../fundamentals/service-limits.md#logs-ingestion-api). Retry after the duration listed in the `Retry-After` header in the response. |
