...
#### On-demand historical backup with an export job

Data export rules only capture records that arrive *after* you enable a rule. To back up records that already exist in your workspace, run an [export job (preview)](../export-job.md). The job exports a single table and time range to Azure Blob Storage in Parquet format, which is more efficient to re-query by using the [externaldata operator](/kusto/query/externaldata-operator?view=azure-monitor&preserve-view=true) or Azure Data Explorer than the five-minute JSON blobs that continuous export produces.
...
| Data export rules |  | :white_check_mark: |  | Protection from data loss because of a regional failure <sup>1</sup> | Enable per table. | Cost of data export + Storage blob or Event Hubs |
