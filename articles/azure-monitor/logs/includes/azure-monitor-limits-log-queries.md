...
| Activity log API query rate | 50 queries per 30 seconds | This [activity log API](../../platform/activity-log.md#view-and-retrieve-the-activity-log) has a separate rate limit from the general Logs query API. Querying activity log from a Log Analytics workspace `AzureActivity` table counts against the general Logs query API limits. |
| Activity log API timeout | 75 seconds | The maximum timeout period for the activity log REST API is 75 seconds. Clients can explicitly set the maximum time they wait for a response before timing out by using the [`Prefer` header](../../platform/activity-log.md#view-and-retrieve-the-activity-log). |
