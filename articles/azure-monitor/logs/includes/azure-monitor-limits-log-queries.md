...
| Cross resource queries | Maximum number of Application Insights resources and Log Analytics workspaces in a single query limited to 100.<br>View Designer doesn't support cross-resource query.<br>The scheduledQueryRules API supports cross-resource query in log alerts.<br>See [Cross-resource query limits](../cross-workspace-query.md#considerations) for details. |
...
| Activity logs API query rate | 50 queries per 30 seconds | This [activity logs API](../../platform/activity-log.md#view-and-retrieve-the-activity-log) has a separate rate limit from the general Logs query API. Querying activity logs from a Log Analytics workspace `AzureActivity` table counts against the general Logs query API limits. |
| Activity logs API timeout | 75 seconds | The maximum timeout period for the [activity logs REST API](../../platform/activity-log.md#view-and-retrieve-the-activity-log) is 75 seconds. Clients can explicitly set the maximum time they wait for a response before timing out by using the [`Prefer` header](../../platform/activity-log.md#view-and-retrieve-the-activity-log). |

Keep these best practices in mind to ensure system responsiveness:
