...
| Nested JSON | Auxiliary table plans only support when ingesting to `dynamic` type columns | Logs ingestion API will return HTTP status code 400 "malformed JSON" error and fail if schema isn't matched. |
