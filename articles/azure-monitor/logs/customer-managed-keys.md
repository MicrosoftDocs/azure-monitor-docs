...
Data ingested in the last 14 days, or recently used in queries, is kept in hot-cache (SSD-backed) for query efficiency. SSD data is encrypted by using Microsoft-managed keys regardless of whether you configure customer-managed keys, but your control over SSD access adheres to [key revocation](#key-revocation).
...
- System-assigned managed identity is simpler and generated automatically with the cluster when you set `identity` `type` to `SystemAssigned`. You use this identity later to grant cluster storage access to your Key Vault for data encryption and decryption.
...
Three types of keys are involved in Storage data encryption:
...
- The **AEK** derives **DEK**s, which are the keys that encrypt each block of data written to disk.
...
Dedicated clusters use their managed identity to access your Key Vault KEK to encrypt data. The managed identity type of the dedicated cluster must match the Key Vault role assignment identity to allow for data encryption and decryption operations. 

Configure the `identity` `type` property to `SystemAssigned` or `UserAssigned` when [creating your cluster](./logs-dedicated-clusters.md#create-a-dedicated-cluster). 
  
  For example, here are the values to add in the request body for creating a cluster with a system-assigned managed identity:
...
> - When updating `SystemAssigned` to `UserAssigned`, [Grant `UserAssign` identity](#grant-key-vault-permissions-to-the-managed-identity) in Key Vault, then update `identity` in the dedicated cluster.
...
For more information on creating a dedicated cluster, see [Create and manage a dedicated cluster](./logs-dedicated-clusters.md#create-a-dedicated-cluster). 
...
It takes some time to complete the propagation of the key. Check the update state by sending a `GET` request on the cluster and look at the `KeyVaultProperties` properties. Your recently updated key should return in the response.
...
Verify that the cluster provisioning state is `Succeeded` before linking workspaces to the cluster. If you link workspaces and ingest data before provisioning, ingested data is dropped and can't be recovered.

Verify the provisioning state via CLI, PowerShell, or REST API as detailed in the [Update dedicated cluster with key identifier details](#update-dedicated-cluster-with-key-identifier-details) section.

### Link workspaces to the dedicated cluster

> [!IMPORTANT]
> Perform this step only after the cluster provisioning. If you link workspaces and ingest data before provisioning, ingested data is dropped and can't be recovered. 
...
* Queries saved in a [query pack](./query-packs.md) aren't stored on a linked Storage Account and can't be encrypted witha customer-managed key. It's recommended to **Save as Legacy query** to protect queries with a customer-managed key.
...
- You must configure the Azure Key Vault as recoverable. These properties aren't enabled by default and should be configured using CLI or PowerShell:<br>
  - [Soft Delete](/azure/key-vault/general/soft-delete-overview).
