...
Set up custom roles for the defined scenario or use the built-in role, **Log Analytics Data Reader**. If you setup custom roles, create one for the network team and another for the security team. Keep in mind, role assignments are additive, so remove any roles that assign read access above the granular RBAC conditions. For example, if Log Analytics Reader was assigned to a user at the resource group level, a granular RBAC role is not effective for them. For more information, see [Configure granular RBAC role creation](granular-rbac-log-analytics.md#role-creation).
