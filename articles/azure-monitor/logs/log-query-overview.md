...
- [Log Analytics Query API](/rest/api/loganalytics/query): Retrieve log data from the workspace from any REST API client. The API request includes a query that's run against Azure Monitor to determine the data to retrieve.
   ...
   > [!IMPORTANT]
   > Starting July 1, 2025, querying log data/events requires the use of TLS 1.2 or higher when using [Log Analytics API endpoints](../fundamentals/azure-monitor-network-access.md#log-analytics-api-endpoints). For more information, see [Secure data in transit](../fundamentals/best-practices-security.md#secure-logs-data-in-transit).
