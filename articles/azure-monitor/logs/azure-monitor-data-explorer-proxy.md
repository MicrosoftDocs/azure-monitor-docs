...
- The **only** commands cross-service queries support are `.show` commands. This capability enables cross-cluster queries to reference an Azure Monitor, Azure Data Explorer, or Azure Resource Graph tabular function directly.</br></br>
    `.show` commands supported with the cross-service query:  </br></br>
    `.show functions`  </br>
    `.show function {FunctionName}`  </br>
    `.show database {DatabaseName} schema as json`  </br></br>
