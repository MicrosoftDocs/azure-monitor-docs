...
Log Analytics simple mode provides a point-and-click interface for analyzing and visualizing log data. [Switch between simple and Kusto Query Language (KQL) modes](#switch-modes), and set Log Analytics to [open in simple mode by default](./log-analytics-overview.md#more-tools).
...
Simple mode lets you [get started quickly by retrieving data from one or more tables with one click](#get-started-in-simple-mode). Then use a set of controls to [explore and analyze the retrieved data](#explore-and-analyze-data-in-simple-mode).

## Get started in simple mode

In simple mode, retrieve logs with one click in either resource or workspace context.
...
After you [get started in simple mode](#get-started-in-simple-mode), use the [top action bar](./log-analytics-overview.md#top-action-bar) to explore and analyze data. The order in which you apply filters and operators affects your query and results. For example, if you apply a filter and then aggregate, Log Analytics applies the aggregation to the filtered data. If you aggregate and then filter, Log Analytics applies the aggregation to the unfiltered data.
...
To change the time range and number of records displayed, use the **Time range** and **Show** selectors. The time range you set is applied at the end of the query and doesn't change the amount of data being queried. For more information about result limit, see [Configure query result limit](#configure-query-result-limit).
...
In Log Analytics simple mode, filter a column to narrow results:
...
1. Select a value to filter by, or enter text or numbers in the **Search** box.
    ...
    If you filter by selecting values from a list, select as many values as needed. If the list is long, you see a **Not all results are shown** message. Scroll to the bottom of the list and select **Load more results** to retrieve more values.
...
In Log Analytics simple mode, search the table for a specific value:
...
In Log Analytics simple mode, aggregate data to summarize values across rows:
...
In Log Analytics simple mode, show or hide columns in the results:
...
In Log Analytics simple mode, sort results by column:
...
| [stdev](/azure/data-explorer/kusto/query/stdev-aggregation-function) | Calculates the standard deviation of a set of values. |
...
When you begin to query logs in simple mode and then switch to KQL mode, the query editor is prepopulated with the KQL query related to your simple mode analysis. Then edit the query and continue working with it.
...
For straightforward queries on a single table, Log Analytics displays the table name at the right of the top action bar in simple mode. For more complex queries, Log Analytics displays **User Query** at the left of the top action bar. Select **User Query** to return to the query editor and modify your query at any time.
...
1. Select one of the preset limits, or enter a custom limit.
    ...
    The maximum number of results the Log Analytics portal experience returns, in both simple mode and KQL mode, is 500,000. However, when you [share a Log Analytics query](./log-analytics-overview.md#more-tools) with an integrated tool, or use the query in a [search job](search-jobs.md), the query limit is set based on the tools you choose.

    Select **Max. limit** to return the maximum number of results provided by any of the tools available on the **Share** window or using a search job. When using a search job, this can return up to 100,000,000 results per result set.
...
|------|-------------|------------|
| Log Analytics | Queries you run in the Azure portal. | 500,000 |
...
| [Search job](search-jobs.md) | Azure Monitor reingests the results of a query your run in search job mode into a new table in your Log Analytics. |100,000,000 per result set |
