...
1. To limit your rule group to a cluster scope, select the **Specific cluster** option:
    ...
    * The default **Cluster name** value is entered for you. You should change this value only if you've changed your cluster label value using [cluster_alias](../containers/prometheus-metrics-scrape-configuration.md#cluster-alias).
...
1. Enter the PromQL **Expression** for the rule using the PromQL-sensitive expression editor box. You can see the results of the expression query visualized in the preview chart. You cfan modify the preview time range to zoom in our out on the expression result history.
...
1. Select the **Severity** of alerts fired by this rule.
1. Enter the **Name** of the rule. This name is the name of alerts fired by the rule.
1. Enter the PromQL **Expression** for the rule using the PromQL-sensitive expression editor box. You can see the results of the expression query visualized in the preview chart. You cfan modify the preview time range to zoom in our out on the expression result history.
1. Select the **Wait for** value for the period between the alert expression first becomes true and until the alert is fired.
...
1. Deploy the template using any deployment method, such as [Azure portal](/azure/azure-resource-manager/templates/deploy-portal), [Azure CLI](/azure/azure-resource-manager/templates/deploy-cli), [Azure PowerShell](/azure/azure-resource-manager/templates/deploy-powershell), or [Rest API](/azure/azure-resource-manager/templates/deploy-rest).
