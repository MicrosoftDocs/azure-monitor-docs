...
This article is part of the guide [Monitor virtual machines and their workloads in Azure Monitor](monitor-virtual-machine.md). [Alerts in Azure Monitor](../alerts/alerts-overview.md) proactively notify you of interesting data and patterns in your monitoring data. Virtual machines have no preconfigured alert rules, so create your own based on data you collect from Azure Monitor Agent. This article presents alerting concepts specific to virtual machines and common alert rules used by other Azure Monitor customers.
...
If you set the target resource of a log search alert rule to a Log Analytics workspace, you have access to all data in that workspace. For this reason, a single rule alerts on data from all machines in the workgroup. This arrangement gives you the option of creating a single alert for all machines. Use dimensions to create a separate alert for each machine.
...
Another benefit of using log search alert rules is the ability to include complex logic in the query for determining the threshold value. Hardcode the threshold, apply it to all resources, or calculate it dynamically based on some field or calculated value. The threshold applies to resources only according to specific conditions. For example, you might create an alert based on available memory but only for machines with a particular amount of total memory.
...
One of the most common monitoring requirements for a virtual machine is to create an alert if it stops running. The best method is to create a metric alert rule in Azure Monitor by using the VM availability metric. For a walk-through on this metric, see [Create availability alert rule for Azure virtual machine](tutorial-monitor-vm-alert-availability.md).

An alert rule is limited to one activity log signal. So for every condition, you must create one alert rule. For example, "starts or stops the virtual machine" requires two alert rules. However, to be alerted when the virtual machine is restarted, you need only one alert rule. 
...
Agent heartbeat alerts depend on telemetry being delivered from the Azure Monitor Agent through the ingestion pipeline before the alert evaluation window closes. Transient agent-side or ingestion latency can cause heartbeat records to arrive after the evaluation window, leading the alert to fire even though the virtual machine is healthy and the agent is sending data normally.

If your goal is to detect whether an Azure virtual machine is running, use the VM availability metric alert, which the Azure platform evaluates and isn't subject to agent or log-ingestion latency.
...
#### Metric alert rules for agent heartbeat

Each Log Analytics workspace includes a metric called **Heartbeat**. Each virtual machine connected to that workspace sends a heartbeat metric value each minute. Because the computer is a dimension on the metric, set an alert when any computer fails to send a heartbeat. Set the **Aggregation type** to **Count** and the **Threshold** value to match the **Evaluation granularity**.

#### Log search alert rules for agent heartbeat
...
Set an alert for high processor utilization by using the host metric or guest performance counters.

#### Metric alert rules for CPU
...
#### Log search alert rules for CPU
...
Set an alert for low available memory by using the host metric or guest performance counters.

#### Metric alert rules for memory

| Target | Metric |
|:---|:---|
| Host | Available Memory Bytes (included in recommended alerts) |
| Windows guest | \Memory\% Committed Bytes in Use<br>\Memory\Available Bytes |
| Linux guest | mem/available<br>mem/available_percent |

#### Log search alert rules for memory

> [!NOTE]
> To scope the alert to one disk, add this condition to the query: `| where parse_json(Tags).["vm.azm.ms/mountId"] == "C:"`
...
Set up alerts for low free disk space or high disk activity by using guest performance counters.

#### Metric alert rules for disk
...
#### Log search alert rules for disk
...
Set an alert on network throughput by using the host metric or guest performance counters.

#### Metric alert rules for network

| Target | Metric |
|:---|:---|
| Host | Network In Total, Network Out Total (included in recommended alerts) |
| Windows guest | \Network Interface\Bytes Received/sec<br>\Network Interface\Bytes Sent/sec |
| Linux guest | net/bytes_recv<br>net/bytes_sent |

#### Log search alert rules for network
