...
* You require permissions to create a data collection rule (DCR) and associate it with the Azure Monitor agent. See [Data Collection Rule permissions](../data-collection/data-collection-rule-create-edit.md#permissions) for details.
...
* See [Dependency Agent requirements](./vminsights-dependency-agent.md) to verify that your operating system is supported by Dependency agent and for network requirements for the Dependency agent.
...
[Data collection rules (DCRs)](../data-collection/data-collection-rule-overview.md) are used by the Azure Monitor agent to specify which data to collect and how it should be processed. When you enable VM Insights, you create a DCR specifically for VM insights and associate it with the Azure Monitor agent on any machines to monitor.
...
2. On the **Monitoring configuration** page, select **Azure Monitor agent** and select a [DCR](#vm-insights-dcr) from the **Data collection rule** dropdown. Only DCRs configured for VM insights are listed.
 ...
 1. If a DCR hasn't already been created for VM insights, Azure Monitor offers to create one with a default Log Analytics workspace and the following settings. You can either accept these defaults or click **Create New** to create a new DCR with different settings. This lets you select a workspace and specify whether to collect processes and dependencies using the [VM insights Map feature](vminsights-maps.md).
    ...
    * **Processes and dependencies** disabled.
    ...
    > [!NOTE]
    > If you select a DCR with Map enabled and your virtual machine is not [supported by the Dependency Agent](./vminsights-dependency-agent.md), Dependency Agent will be installed and  will run in degraded mode.
...
* Dependency agent for Linux or Windows if you want to enable the Map feature. See [Dependency Agent requirements](./vminsights-dependency-agent.md) for details. 
...
> [!NOTE]
> If you associate a DCR with the Map feature enabled to a machine on which Dependency Agent isn't installed, the Map view won't be available. To enable the Map view, set `enableAMA property = true` in the Dependency Agent extension when you [install Dependency Agent](./vminsights-dependency-agent.md).

<br>
<details>
<summary><b>Expand to view ARM (JSON) files</b></summary>
...
<br>
<details>
<summary><b>Expand to view Bicep files</b></summary>
...
The ARM (JSON) and Bicep templates described in this section can be deployed using any method to install ARM (JSON) or Bicep templates.

See the Azure Resource Manager documentation for details on deploying templates.
