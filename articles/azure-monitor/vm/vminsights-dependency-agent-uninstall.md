...
- If you use [Azure Policy](./vminsights-enable-policy.md) to enable VM insights, you might need to change policy assignments to prevent compliance problems or reinstallation of the Dependency Agent after it's removed. See [Migrate Dependency Agent policy and initiative assignments](./dependency-agent-migrate-policy.md).
