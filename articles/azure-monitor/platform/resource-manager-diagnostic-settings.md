...
The following sample creates a diagnostic setting for an Azure Recovery Services vault by adding a resource of type `microsoft.recoveryservices/vaults/providers/diagnosticSettings` to the template. This example specifies the collection mode as described in [Azure resource logs](./resource-logs.md#destinations&tabs=log-analytics). Specify `Dedicated` or `AzureDiagnostics` for the `logAnalyticsDestinationType` property.
