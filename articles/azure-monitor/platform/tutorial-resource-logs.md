...
Azure Monitor stores log data in a [Log Analytics workspace](../logs/log-analytics-workspace-overview.md). If you already created a workspace in your subscription, use that one. The default workspace in each Azure subscription is also an option.

To create a new Log Analytics workspace, use the following procedure. To use an existing workspace, move to the next section.

1. In the Azure portal, under **All services**, select **Log Analytics workspaces**.
...
1. On the **Basics** tab, select a subscription, resource group, and region for the workspace. These values don't need to be the same as the resource being monitored. Provide a name that's unique within the resource group.
...
1. Select **Save** to save the diagnostic settings.

## Generate sample data

It takes a few minutes for the diagnostic setting to start collecting data. To generate sample data, perform an operation on the resource that generates logs. For example, if you selected a storage account, upload some files to it and then view those files. Each of these actions generates a log entry that is sent to the Log Analytics workspace.

To verify that data is flowing, run a log query against the resource's logs as described in the next section. Expect rows to appear within about 10 minutes of generating the sample data.

## Use a log query to retrieve logs

You retrieve data from a Log Analytics workspace by using a log query written in Kusto Query Language (KQL). Many Azure services provide a set of [curated queries](../reference/queries-by-table.md).

Log data can take several minutes to appear after it's generated.

1. Select **Logs** from your resource's menu. Log Analytics opens with the **Queries hub** window that includes prebuilt queries for your resource type.
...
1. Browse through the available queries. Identify one to run and select **Run**. The query is added to the query window and the results are returned.
...
1. If the query you chose creates a chart, select the **Results** tab to view the results in a table format.
