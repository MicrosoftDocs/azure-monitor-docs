---
title: Create Diagnostic Settings at Scale by Using Azure Policy and Initiatives
...
- To create policy definitions, initiatives, and assignments, you need the **Resource Policy Contributor** role, or a role that includes these permissions, such as **Owner**, at the scope where you work.
- For `deployIfNotExists` remediation, the policy assignment requires a managed identity. Grant that identity a role with permission to create diagnostic settings at the assignment scope, such as **Monitoring Contributor** or **Log Analytics Contributor**.
...
|----------------|----------|
| `allLogs` | All available logs for the resource. |
| `audit` | All audit logs for the resource. |

Use a category group to define a policy that's dynamically updated as new log categories are added to the group.

## Create a custom policy definition

For resource types that don't have a built-in policy, create a custom policy definition. Create the policy manually in the Azure portal by copying an existing built-in policy and then modifying it for your resource type. Alternatively, create the policy programmatically by using a script in the PowerShell Gallery.

The script [`Create-AzDiagPolicy`](https://www.powershellgallery.com/packages/Create-AzDiagPolicy) creates policy files for a particular resource type. Install the files by using PowerShell or the Azure CLI. Use the following procedure to create a custom policy definition for diagnostic settings:
...
1. Run the script by using the parameters to specify where to send the logs. Specify a subscription and resource type at the prompt.
    ...
    For example, to create a policy definition that sends logs to a Log Analytics workspace and Azure Event Hubs, use the following command:

    ```azurepowershell
    Create-AzDiagPolicy.ps1 -ExportLA -ExportEH -ExportDir ".\PolicyFiles"
    ```

    Alternatively, specify a subscription and resource type in the command. For example, to create a policy definition that sends logs to a Log Analytics workspace and Azure Event Hubs for SQL Server databases, use the following command:
...
* Modify the initiative when you need to add a new resource type or destination. For example, your initial requirements might be to send data only to a Log Analytics workspace, but later you want to add Azure Event Hubs. Modify the initiative instead of creating new assignments.
...
* Instead of specifying the details for the Log Analytics workspace and Azure Event Hubs for policy definitions included in the initiative, use a common initiative parameter. This parameter allows you to specify a common value for all policy definitions and change that value if necessary.
...
Initiative parameters let you specify the workspace or any other details once for all of the policy definitions in the initiative.
...
Azure Policy applies the initiative to each resource when you create the resource. A [remediation task](/azure/governance/policy/how-to/remediate-resources) deploys the policy definitions in the initiative to existing resources and creates diagnostic settings for those resources.
...
After the assignment and any remediation task complete, confirm that diagnostic settings exist on your resources:

1. In the Azure portal, go to a resource in the assignment scope, and then select **Diagnostic settings** under **Monitoring**. Confirm that a setting sends logs to the destinations that you configured.

1. Alternatively, list the diagnostic settings for a resource by using the Azure CLI:
