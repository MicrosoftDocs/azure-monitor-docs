...
- **Per-processor pipeline metrics** — `processor_incoming_items`, `processor_outgoing_items`, and `processor_duration` now report the item throughput and processing time of every processor in each pipeline, giving you finer-grained visibility into pipeline performance. `processor_duration` is now reported in milliseconds.
