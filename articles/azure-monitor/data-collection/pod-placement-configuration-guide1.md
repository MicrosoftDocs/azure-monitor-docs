Pod Placement Configuration Guide
...
<th>string</th>
<th>Yes</th>
<th>The node attribute/label to match (e.g., zone, gpu, team, pipeline)</th>
</tr>
</thead>
<tbody>
<tr>
...
<td>string</td>
<td>Yes</td>
<td><p>Matching logic: In, NotIn,</p>
<p>Exists, DoesNotExist</p></td>
</tr>
<tr>
<td>values</td>
<td>array of strings</td>
<td>Conditional</td>
<td>Values to match for the capability. Required for In and NotIn, not used for</td>
</tr>
</tbody>
</table>

Exists and DoesNotExist **Supported Operators:**
...
- **operator** field is required and must be one of: In, NotIn, Exists, DoesNotExist • **values** field requirements depend on the operator:

> o **Required for In and NotIn**: Must provide a non-empty array of strings o **Not allowed for Exists and DoesNotExist**: Must be omitted or empty

- Invalid operator/values combinations will result in configuration errors Distribution Validation

- **maxInstancesPerHost** must be a positive integer

- Currently only 1 is supported for strict isolation

- Other values will be rejected during configuration **Example Invalid Configurations:**
...
maxInstancesPerHos integer No Maximum instances per Property Type Required Description

t node **for this specific**

> **pipelineGroup**. Currently supports 1 (strict isolation) or unset (no limit).
...
- Multiple replicas of the **same** pipelineGroup cannot share a node (when maxInstancesPerHost: 1) **Example:**
...
Node 1: PipelineGroupA-replica1 + PipelineGroupA-replica2 Scheduling Enforcement
...
Example 4: Strict Isolation with Node Labeling (Recommended for High-Scale) Combine node targeting with strict isolation - exactly one instance per node:
...
Label your nodes with: kubectl label nodes \<node-name\> workload-type=telemetry-processing
...
> o Ensure capability names match actual node labels o Confirm operator and values are correct o Check for typos in label names or values

4.  **Check anti-affinity conflicts:**

> If using maxInstancesPerHost: 1, ensure you have enough eligible nodes for all replicas:
>
> *\# Count eligible nodes* kubectl get nodes -l \<your-label-selector\>
...
affinity**:** nodeAffinity**:** requiredDuringSchedulingIgnoredDuringExecution**:** nodeSelectorTerms**:** **-** matchExpressions**:** **-** key**:** \<capability\>

operator**:** \<In\|NotIn\|Exists\|DoesNotExist\> values**:** **\[**\<values\>**\]**
...
- labelSelector**:** matchExpressions**:** **-** key**:** pipeline operator**:** In values**:**

- \<pipelineGroupName\> topologyKey**:** "kubernetes.io/hostname"
