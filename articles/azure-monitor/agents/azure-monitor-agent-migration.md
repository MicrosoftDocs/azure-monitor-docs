...
> - **Installation:** You can't install the Log Analytics agent from the Azure portal, but offline and extension-based installation still work.
...
- For non-Azure and on-premises servers, [install the Azure Arc Connected Machine agent](/azure/azure-arc/servers/agent-overview), which makes your on-premises servers visible to Azure as targetable resources at no extra cost.
...
| Tool | Purpose |
|---|---|
| [Azure Monitor Agent Migration Helper workbook](./azure-monitor-agent-migration-helper-workbook.md) | A workbook-based Azure Monitor solution to inventory agents, audit workspaces, identify dependent services, and track migration progress. |
...
| Are you using System Center Operations Manager (SCOM)? | If you plan to continue using SCOM, evaluate [SCOM Managed Instance](/system-center/scom/operations-manager-managed-instance-overview). You can keep the Log Analytics agent on machines SCOM manages. |
| How are agents deployed today? | If you use automated deployment for the Log Analytics agent, stop deploying it to new servers to prevent your migration backlog from growing. |
...
| **Microsoft Defender for Cloud** | If you use Defender for Servers Plan 2, change your agent deployment in Defender for Cloud from the Log Analytics agent to agentless scanning. If you use Defender for Cloud to collect security events, create a custom data collection rule to collect them. |
...
1. **Disable Log Analytics agent data collection during testing.** To avoid double ingestion, remove the Log Analytics agent's workspace configurations on pilot servers without uninstalling the agent. For more information, see [Configure data sources for the Log Analytics agent](/azure/azure-monitor/agents/agent-data-sources#configure-data-sources).

1. **Deploy at scale by using Azure Policy.** Use built-in policies to deploy Azure Monitor Agent extensions and data collection rule associations at scale. It also automatically deploys to new machines. For more information, see [Use Azure Policy to install and manage the Azure Monitor Agent](./azure-monitor-agent-policy.md).
...
1. **Compare ingested data.** Run KQL queries against your Log Analytics workspace to compare the data ingested by the Log Analytics agent with data from Azure Monitor Agent. For example, query the `Heartbeat` table and filter by `Category` to confirm Azure Monitor Agent heartbeats are arriving.
