...
This article helps you troubleshoot errors with the Log Analytics VM extension for Windows and Linux Azure VMs and suggests possible solutions to resolve them.
...
1. From the list, check whether the Log Analytics extension is enabled. For Linux, the agent is listed as **OMSAgentforLinux**. For Windows, the agent is listed as **MicrosoftMonitoringAgent**.
   <!-- convertborder later -->
...
If the Microsoft Monitoring Agent VM extension isn't installing or reporting, follow these steps:
...
1. Review the Microsoft Monitoring Agent setup log files in `C:\WindowsAzure\Logs\Plugins\Microsoft.EnterpriseCloud.Monitoring.MicrosoftMonitoringAgent\1.0.18053.0\`. This path varies by agent version.
...
If the Log Analytics agent for Linux VM extension isn't installing or reporting, follow these steps:
