...
As one of the migration options, customers can migrate to [Azure Monitor Agent (AMA)](./agents-overview.md) configured with [Data Collection Rules (DCRs)](./data-collection-rule-overview) to continue collecting guest OS logs and performance data and to unlock multi-destination routing, centralized and more secure configuration, and at-scale management. **Note**: DCRs provide transformations (ingestion-time filtering, parsing) to reduce cost and tailor schemas—something not available in WAD/LAD.
...
-   For fleet view, use Azure Resource Graph query from the [WAD/LAD overview article](./diagnostics-extension-overview) to find extensions by publisher == "Microsoft.Azure.Diagnostics". [learn.microsoft.com]
...
iii) Prioritize what to keep / drop - Use this moment to reduce noise and unused data types, if any. With DCR transformations, you can additionally [filter to essential signals](../data-collection/data-collection-rule-overview#transformations).

### Phase 2: Design DCRs
Create DCRs that replicate the WAD/LAD intent:

- [Standard VM data sources](../vm/data-collection#add-data-sources) like Windows events (e.g., System, Application, Security), Linux syslog, performance counters. 
- [Custom text logs](../vm/data-collection-log-text) (same file paths wildcards; ensure custom tables exist; often requires a DCE) sent to low-cost [Auxiliary tier](../logs/data-platform-logs.md#table-plans) in Log Analytics. [learn.microsoft.com]
- [ADX-destination routing] (../vm/send-fabric-destination#create-a-data-collection-ruledocs.azure.cn)
...
i) Prepare prerequisites for [AMA installation](./azure-monitor-agent-requirements) and [DCR association](./data-collection/data-collection-rule-create-edit?tabs=cli#permissions), including verifying [supported OS], [network settings](./azure-monitor-agent-network-configuration) 
ii) Ensure you have Log Analytics workspace setup with [Auxiliary tier](../logs/data-platform-logs.md#table-plans) as needed.
iii) (Recommended) Use Azure Policy [built‑in initiatives to install AMA](./azure-monitor-agent-policy) on Windows/Linux and associate DCRs. Using policy ensures new machines are auto-onboarded and existing machines are remediated, aligning to the migration best practices.
