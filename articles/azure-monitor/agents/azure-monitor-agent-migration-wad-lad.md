...
As one of the migration options, customers can migrate to [Azure Monitor Agent (AMA)](./agents-overview.md) configured with [Data Collection Rules (DCRs)](./data-collection-rule-overview.md) to continue collecting guest OS logs and performance data and to unlock multi-destination routing, centralized and more secure configuration, and at-scale management. 
> [!NOTE]
...
4. Prioritize what to keep / drop - Use this moment to reduce noise and unused data types, if any. With DCR transformations, you can additionally [filter to essential signals](../data-collection/data-collection-rule-overview.md#transformations).
...
2. Stop/Remove the Microsoft.Azure.Diagnostics extension from each VM after AMA data parity is confirmed; this is explicitly recommended to avoid double ingestion and extra costs.
