...
Display all error messages from the HorizonDB log.
...
Display all fatal messages from the HorizonDB log.
...
Search for deadlock events in the HorizonDB log.
...
Search for server shut down and server ready events in the HorizonDB log.
...
Search for received connections messages in the HorizonDB log.
...
Search for authorized connections messages in the HorizonDB log.
...
Search for unauthorized (failed) connections in the HorizonDB log.
...
Search for lock contention in HorizonDB log. It requires parameter log_lock_waits=ON.
...
Search for autovacuum events in HorizonDB log. It requires parameter 'log_autovacuum_min_duration' enabled.
...
Search for all audit events in HorizonDB log. It requires audit logs to be enabled [https://learn.microsoft.com/azure/postgresql/flexible-server/concepts-audit].
