...
Which dashboards are available and what data they show depends on how your virtual machines are monitored. Some dashboards use host platform metrics that are available for all Azure virtual machines. Others use the preview OpenTelemetry metrics-based VM experience and the classic logs-based experience.
...
**Azure | Resources | Virtual Machines - Platform Metrics** works for all Azure virtual machines because it uses host [platform metrics](../metrics/data-platform-metrics.md) automatically collected by Azure. Use this dashboard when you want a broad operational view of VM health and resource usage without depending on guest-level monitoring data.

### OpenTelemetry dashboards

The following dashboards are designed for virtual machines that use the [preview VM monitoring experience](../vm/metrics-opentelemetry-guest.md) based on OpenTelemetry data sent to an Azure Monitor workspace:
