...
1. Under **Destination details**, select one or more destinations, fill out details, and select **Save**.
   ...
   | Event hub               | Stream to an event hub                 | Select a **subscription** and an existing Azure Event Hubs **namespace**. Optionally also choose an existing **event hub**. Choose an **event hub policy** from the list. Only event hubs in the same region as the Grafana dashboard are displayed in the dropdown menu. |
