...
Sign in to the Azure CLI by running the [az login](/cli/azure/reference-index?view=azure-cli-latest#az-login) command and replace `<client-id>`, `<client-secret>`, and `<tenant-id>` with the application (client) ID, client secret, and tenant ID collected in the previous step:
...
1. Delete Azure Monitor dashboards with Grafana:
   1. In the Azure portal, in Azure Monitor dashboards with Grafana, select **Overview** from the left menu.
   1. Select **Delete**.
