...
|Template ID comes from| Specify the ID of the template to open. A community template from the gallery is the most common case. Prefix the path to the template with `Community-`, like `Community-Workbooks/Performance/Apdex` for the `Workbooks/Performance/Apdex` template. If it's a link to a saved workbook or template, use the full path to the Azure resource ID of that item, for example, `/subscriptions/a0a0a0a0-bbbb-cccc-dddd-e1e1e1e1e1e1/resourceGroups/rgname/providers/microsoft.insights/workbooks/b1b1b1b1-cccc-dddd-eeee-f2f2f2f2f2f2`. |
