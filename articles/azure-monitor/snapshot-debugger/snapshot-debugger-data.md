---
title: Snapshot Debugger data in Application Insights
description: Learn how to view debug snapshots from the Snapshot Debugger in the Azure portal or Visual Studio to examine the call stack and inspect variables.
...
After an exception occurs in your application and Snapshot Debugger creates a snapshot, you can view snapshots in the [Azure portal](https://portal.azure.com) within 5 to 10 minutes.

<a name="from-the-failures-pane"></a>
...
1. From the list of samples, select an operation or exception to open the **End-to-End Transaction Details** page. From here, select the exception event you want to investigate.
   ...
   - If you don't see this option, a snapshot might not be available. See the [troubleshooting guide](./snapshot-debugger-troubleshoot.md#use-the-snapshot-health-check).
...
<a name="from-the-code-optimizations-consolidated-overview"></a>
...
1. Use the filters to narrow results to your specific Azure subscription and resource. 
...
1. Select **Download Snapshot** in the **Debug Snapshot** page to download a `.diagsession` file, which Visual Studio Enterprise can open.
...
1. Select **Debug Managed Code** to start debugging the snapshot. The snapshot opens to the line of code that threw the exception.

   :::image type="content" source="./media/snapshot-debugger/open-snapshot-visual-studio.png" alt-text="Screenshot showing the debug snapshot in Visual Studio." lightbox="./media/snapshot-debugger/open-snapshot-visual-studio.png":::
