...
For log search alert rules that have a custom JSON payload defined, enabling the common alert schema reverts the payload schema to the one described in [Common alert schema](../alerts/alerts-common-schema.md#log-search-alerts-context-fields). If you want to have a custom JSON payload defined, the webhook can't use the common alert schema.
