...
| Query-based metric alerts | Create a metric alert rule on Prometheus metrics or custom OTel metrics stored in an Azure Monitor Workspace using query-based metric alert condition with PromQL. Similar to Prometheus alerts but managed as individual resources instead of rule groups. | Currently in public preview. |
...
| Azure Managed Redis | "Microsoft.Cache/redis" | Yes | Yes | Yes |
