...
1. In the **Alert logic** section, select values for these fields:
    ...
    > * The query uses the `AzureDiagnostics` table and filters on a `Category` value that has an equivalent resource-specific table – for example, `AzureDiagnostics | where Category == "AZFWThreatIntel"`. Query the resource-specific table directly instead, such as `AZFWThreatIntel`. For more information, see [Azure diagnostics mode](../logs/resource-logs.md?tabs=log-analytics#azure-diagnostics-mode).
