...
| Log alerts | 5,000 active alert rules per subscription. Out of which, 100 active alert rules with a 1-minute frequency.<br>1,000 active alert rules per resource.| Call support. |
| Log alerts | Each stateless alert rule can trigger up to 6,000 alerts per evaluation.<br>Each stateful alert rule can trigger up to 300 alerts per evaluation.<br>Up to 5,000 fired stateful alerts at a time per alert rule.<br>Stateful alert rules can be configured with a frequency of up to 12 hours.<br>The combined size of all data in the log alert rule properties can't exceed 64 KB.<br>The Kusto query results can't exceed more than 20 MB. <br> 500 active alert rules per managed identity on Log Analytics or ADX  workspace. <br> 50 active alert rules per managed identity on Azure resource graph workspace. | Same as default. |
...
| Resource                                                                                | Default limit                           | Maximum limit   |
|-----------------------------------------------------------------------------------------|-----------------------------------------|-----------------|
| [Alerts - Get Summary](/rest/api/monitor/)           | 50 calls per minute per subscription    | Same as default |
| [Alerts - Get All](/rest/api/monitor/) (not "Get By ID") | 100 calls per minute per subscription   | Same as default |
| [All other alerts calls](/rest/api/monitor/)                     | 1,000 calls per minute per subscription | Same as default |
