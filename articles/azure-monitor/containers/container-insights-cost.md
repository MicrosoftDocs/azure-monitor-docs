...
|:---|:--|
| Tables | Manually modify the DCR if you want to select individual tables to populate other than the logs preset groups. For example, you may want to collect **ContainerLogV2** but not collect **KubeEvents** which is included in the same log preset. <br><br>See [Stream values](./kubernetes-monitoring-enable-aks.md#stream-values) for a list of the streams to use in the DCR. |
