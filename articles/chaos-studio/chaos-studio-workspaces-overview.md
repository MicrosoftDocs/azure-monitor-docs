...
**Identity** is the managed identity the Workspace uses to execute Actions against your resources. The identity serves as a security boundary — it ensures that only an authorized principal with the right RBAC role assignments can run Actions against specific resources. You control the blast radius by granting the identity roles on exactly the resources you want to test, and nothing more. You can use a system-assigned managed identity (created with the Workspace) or a user-assigned managed identity (shared across Workspaces). Chaos Studio can assign the required roles automatically during Workspace creation, or you can configure them manually.
...
When you create a Workspace, you select one of three scope types:
...
For details on the Scenario catalog, including which Actions each Scenario composes and which resources it targets, see [Scenarios in Azure Chaos Studio](chaos-studio-scenarios.md).
...
Over time, Chaos Studio will expand the Scenario catalog to cover more outage patterns. As that happens, starting from a Workspace becomes the fastest way to get resilience coverage for the most common failure modes.
