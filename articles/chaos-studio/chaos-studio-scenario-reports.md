...
- **Operational evidence** — After a resilience test, you have a concrete artifact showing what was tested, against which resources, and what the outcome was. Pair the report with your own application health checks and monitoring data to assess whether the system recovered within your target RTO.
...
- **Post-incident retrospectives** — When you reproduce an outage pattern after an incident, the Scenario report documents whether the fix you applied actually improved resilience.
...
| Status | The overall outcome: **Succeeded**, **Failed**, or **Cancelled**. |
| Started At | The timestamp when execution began. |
| Completed At | The timestamp when all actions finished or the run was cancelled. |
...
| Display Name | A human-readable description of what the Action does (for example, "Force shutdown VMSS instances in target zone"). |
...
Actions with a **Skipped** status are normal in many Scenarios. A Zone Down Scenario, for example, may include Actions for resource types that exist in the Scenario definition but aren't present in your Workspace scope. Skipped Actions don't affect the overall run status.
...
The timeline helps you understand the temporal relationship between Actions. In a Compute Zone Down Scenario, for example, you can see whether the VMSS shutdown and the load balancer backend removal happened simultaneously or sequentially, and how much total time the combined disruption lasted.
