...
Chaos Studio supports two fault types. **Service-direct faults** call Azure management APIs to act on resources externally, such as shutting down a VM or failing over a database. **Agent-based faults** run inside the VM to inject conditions that management APIs can't replicate, like sustained memory pressure or targeted network latency to a specific endpoint.
