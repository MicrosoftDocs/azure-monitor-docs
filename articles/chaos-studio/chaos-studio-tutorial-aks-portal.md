---
title: Create AKS Chaos Mesh faults in the portal (classic)
description: Enable an AKS target, choose a Chaos Mesh pod fault in the portal designer, and grant permissions for Azure Chaos Studio Experiments (classic).
...
# Create AKS Chaos Mesh faults in the portal (classic)

Use this portal guide for AKS Chaos Mesh faults with Azure Chaos Studio Experiments (classic). For current-model node resilience tests, see [AKS resilience testing with Chaos Studio Workspaces](chaos-studio-aks-guidance.md) or the [AKS zone-failure Scenario tutorial](chaos-studio-tutorial-sample-app.md).
...
- An AKS cluster with a Linux node pool. If you don't have an AKS cluster, see the AKS quickstart that uses the [Azure CLI](/azure/aks/learn/quick-kubernetes-deploy-cli), [Azure PowerShell](/azure/aks/learn/quick-kubernetes-deploy-powershell), or the [Azure portal](/azure/aks/learn/quick-kubernetes-deploy-portal).
...
1. Run the following commands in an [Azure Cloud Shell](/azure/cloud-shell/overview) window where you have the active subscription set to be the subscription where your AKS cluster is deployed. Replace `MyManagedCluster` and `MyResourceGroup` with the name of your cluster and resource group.
